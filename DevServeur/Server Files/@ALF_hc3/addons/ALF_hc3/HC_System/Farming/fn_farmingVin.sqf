...
if(_obj getVariable ["vin",false]) exitWith {
	["INFO","Tu as récupéré le champagne.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
	["ALF_Vin",true] remoteExec ["life_fnc_handleItem",_unit];
...
if(_unit distance _obj < 8) then {
	["ALF_Vin",true] remoteExec ["life_fnc_handleItem",_unit];
	_obj setVariable ["vin",false];
	["INFO","Tu as récupéré la bouteille de champagne.","success"] remoteExec ["ALF_fnc_doMsg",_unit];
} else {
	_obj setVariable ["vin",true];
};
