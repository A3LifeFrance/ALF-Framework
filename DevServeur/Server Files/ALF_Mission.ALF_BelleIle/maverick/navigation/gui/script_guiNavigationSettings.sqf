	...
	case "onLoad": {
...

		{
			...
			// Check if the sounds are configured in the CfgSounds
			{
				...
				if (!(isClass (missionConfigFile >> "CfgSounds" >> _soundClassname)) && {!(isClass (configFile >> "CfgSounds" >> _soundClassname))}) exitWith {
					_isValid = false;
				};
