...
if("ALF_Boiteaoutils" in (magazines player)) then {
	_R1 buttonSetAction "[1,life_vInact_curTarget] spawn ALF_fnc_repairBoat; closeDialog 0;";
} else {
...
if("ALF_KitMecatechnic" in (magazines player)) then {
	_R2 buttonSetAction "[2,life_vInact_curTarget] spawn ALF_fnc_repairBoat; closeDialog 0;";
} else {
	_R2 ctrlEnable false;
};

_R3 ctrlSetText "Pousser";
_R3 buttonSetAction "[life_vInact_curTarget] spawn life_fnc_pushObject; closeDialog 0;";
if (alive _veh && {local _veh} && {crew _veh isEqualTo []}) then {_R3 ctrlShow true;} else {_R3 ctrlShow false};
