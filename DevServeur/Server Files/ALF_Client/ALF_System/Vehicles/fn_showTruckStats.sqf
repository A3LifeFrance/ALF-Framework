...
switch (true) do {
	...
	case (_dammage > 0.33 && _dammage < 0.66) : {
		...
		_MainTruckOrange ctrlShow true;
		_MainTruckRouge ctrlShow false;
	};
	case (_dammage > 0.66) : {
		_MainTruckVert ctrlShow false;
		_MainTruckOrange ctrlShow false;
		_MainTruckRouge ctrlShow true;
	};
};
...
private _hitlm = _veh getHitPointDamage "HitLMWheel";
switch (true) do {
	case (_hitlm < 0.33) : {
		_LMTruckVert ctrlShow true;
		_LMTruckOrange ctrlShow false;
		_LMTruckRouge ctrlShow false;
	};
	case (_hitlm > 0.33 && _hitlm < 0.66) : {
		_LMTruckVert ctrlShow false;
		_LMTruckOrange ctrlShow true;
		_LMTruckRouge ctrlShow false;
	};
	case (_hitlm > 0.66) : {
		_LMTruckVert ctrlShow false;
		_LMTruckOrange ctrlShow false;
		_LMTruckRouge ctrlShow true;
	};
};
private _hitrm = _veh getHitPointDamage "HitRMWheel";
switch (true) do {
	case (_hitrm < 0.33) : {
		_RMTruckVert ctrlShow true;
		_RMTruckOrange ctrlShow false;
		_RMTruckRouge ctrlShow false;
	};
	case (_hitrm > 0.33 && _hitrm < 0.66) : {
		_RMTruckVert ctrlShow false;
		_RMTruckOrange ctrlShow true;
		_RMTruckRouge ctrlShow false;
	};
	case (_hitrm > 0.66) : {
		_RMTruckVert ctrlShow false;
		_RMTruckOrange ctrlShow false;
		_RMTruckRouge ctrlShow true;
	};
};
