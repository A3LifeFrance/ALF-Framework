...
if("ALF_Boiteaoutils" in (magazines player)) then {
	_R1 buttonSetAction "[1,life_vInact_curTarget] spawn ALF_fnc_repairTruck; closeDialog 0;";
} else {
...
if("ALF_KitMecatechnic" in (magazines player)) then {
	_R2 buttonSetAction "[2,life_vInact_curTarget] spawn ALF_fnc_repairTruck; closeDialog 0;";
} else {
...
if("ALF_Roue" in (magazines player)) then {
	_R3 buttonSetAction "[0,life_vInact_curTarget] spawn ALF_fnc_repairTruck; closeDialog 0;";
} else {
