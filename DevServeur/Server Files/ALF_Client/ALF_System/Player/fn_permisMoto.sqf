...
*/
if !(license_driver) exitWith {["Moniteur Auto-Ecole", "Tu n'as pas le permis de conduire.", "danger", false] spawn ALF_fnc_doMsg;};
if (license_moto) exitWith {["Moniteur Auto-Ecole", "Tu as dejà le permis Moto.", "danger", false] spawn ALF_fnc_doMsg;};
...
if(_action) then {
	...
	_vehicle = createVehicle ["V_ALF_Diavel_Ecole", [0,0,2000], [], 0, "NONE"];
	...
	["Moniteur Auto-Ecole", "Le moniteur est prêt a te guider a distance, tu peux monter sur la moto.", "warning", false] spawn ALF_fnc_doMsg;

	for "_i" from 0 to 1 step 0 do {
		if (time - _passageTime > 300) exitWith {_exit = true;};
		if (typeOf (vehicle player) isKindOf "V_ALF_Diavel_Ecole") exitWith {_etape1 = true;};
		if (player distance _vehicle > 100) exitWith {_exit = true;};
		sleep 1;
	};
	if (_exit) exitWith {hint "Tu as râte ton permis, le moniteur s'est déconnecté."};
	...
	["G", "15m", "Garez vous sur le parking et arrêtez la moto."] spawn ALF_fnc_doGps;
	...
	sleep 2;
	if (_erreur < 6) then {
		["Moniteur Auto-Ecole", "Vous avez votre permis moto, felicitation !", "success", false] spawn ALF_fnc_doMsg;
		...
		["Moniteur Auto-Ecole", "Vous avez fait trop d'erreurs, vous n'avez pas eu votre permis moto..", "danger", false] spawn ALF_fnc_doMsg;
		life_permis_action = false;
	};
};
