...
if (life_permis_time) exitWith {["Moniteur Auto-Ecole", "Tu ne peux pas passer ton stage de récupération tout de suite. Tu ne peux passer de stage que toute les 48H.", "danger", false] spawn ALF_fnc_doMsg;};

_price = 925;

_action = [format["Veux-tu passer ton stage de récupération de points pour %1€. Attention, epreuve sous 5 erreurs eliminatoires.",_price],"Inscription au stage","OUI","NON"] call BIS_fnc_guiMessage;
...
if(_action) then {
	...
	sleep 2;
	if (_erreur < 6) then {
		["Moniteur Auto-Ecole", "Vous avez récupéré 6 points, felicitation !", "success", false] spawn ALF_fnc_doMsg;
		license_driver = true;
		[1] call SOCK_fnc_updatePartial;
		private _sum = life_permis + 6;
		if(_sum > 12) then {
			_sum = 12;
		};
		life_permis = _sum;
		...
		["Moniteur Auto-Ecole", "Vous avez fait trop d'erreurs, vous n'avez pas eu votre stage..", "danger", false] spawn ALF_fnc_doMsg;
		life_permis_action = false;
	};
};
