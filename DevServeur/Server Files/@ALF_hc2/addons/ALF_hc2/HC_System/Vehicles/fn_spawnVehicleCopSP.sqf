...
if (_type isEqualTo -1) then {
	_vInfo = [format["SELECT id, classname, type, alive, active, plate, gear, fuel, damage, peinture, id_garage FROM vehicles_cop WHERE id='%1'",_vid],2] call HC_fnc_asyncCall;
} else {
	_vInfo = [format["SELECT id, classname, type, alive, active, plate, gear, fuel, damage, peinture, id_garage FROM vehicles_med WHERE id='%1'",_vid],2] call HC_fnc_asyncCall;
};
...
if (_type isEqualTo -1) then {
	[format["UPDATE vehicles_cop SET active='1', pid='%2' WHERE id='%1'",_vid, getPlayerUID _unit],1] spawn HC_fnc_asyncCall;
} else {
	[format["UPDATE vehicles_med SET active='1', pid='%2' WHERE id='%1'",_vid, getPlayerUID _unit],1] spawn HC_fnc_asyncCall;
};
	
private _gear = [_vInfo select 6] call HC_fnc_mresToArray;
private _damage = [_vInfo select 8] call HC_fnc_mresToArray;
private _plate = [(_vInfo select 5)] call HC_fnc_mresToArray;
