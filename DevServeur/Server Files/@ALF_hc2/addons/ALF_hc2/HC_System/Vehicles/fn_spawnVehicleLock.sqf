...
private _vInfo = [format["SELECT id, classname, type, pid, alive, active, plate, gear, fuel, damage, insure, insurecount, peinture FROM vehicles WHERE id='%1' AND pid='%2'",_vid,_pid],2] call HC_fnc_asyncCall;
...
[format["UPDATE vehicles SET active='1', lockveh='0' WHERE pid='%1' AND id='%2'",_pid,_vid],1] spawn HC_fnc_asyncCall;
...
private "_vehicle";
if(_sp isEqualType "") then {
	_vehicle = (_vInfo select 1) createVehicle [0,0,1000];
...
//On donne les clés
[_vehicle] remoteExecCall ["life_fnc_addVehicle2Chain",(playableUnits select {(([(getPlayerUID _x),_keys] call TON_fnc_index) > -1) OR {(getPlayerUID _x) isEqualTo (_vInfo select 3)}})];
[_pid,_vehicle,1] call HC_fnc_keyManagement;

_vehicle lock 2;
_vehicle setVariable ["vehicle_info_owners",_pid,true];
_vehicle setVariable ["dbInfo",[_vInfo select 3,_plate,(_vInfo select 10),(_vInfo select 11),0]];
...
if((_vInfo select 1) in ["V_ALF_Megane_G_Gendarmerie","V_ALF_Focus_Gendarmerie","V_ALF_Berlingo_Gendarmerie","V_ALF_Subaru_Gendarmerie","V_ALF_GIGN","V_ALF_RS_01_Gendarmerie","V_ALF_Titan_Gendarmerie","V_ALF_Passat_GND","V_ALF_DV15_Gendarmerie","V_ALF_Vito_Gendarmerie"]) then {
...
["Fourriere","Le véhicule est sortie de la fourrière.","warning"] remoteExec ["ALF_fnc_doMsg",_unit];
